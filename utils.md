...
<style> /* Styles specific to this page */
	...
	table { /* v5 → v6 migration lazy bodge */
		display:	table;
	}
...
The following is a compilation of tools I frequently use on a bunch of other
sites.
...
<script>
	...
	function Go() {
		var picker = document.querySelector("#funcpicker");
...
<!-- Directly lifted table markup from v5 because lazy. -->
<table>
	<tr><th colspan="2">
...
<script>
	setTimeout(function() {
		var func = (new URLSearchParams(window.location.search)).get("func");
		var picker = document.querySelector("#funcpicker");
		var options = [];
		for(var i = 0; i < picker.options.length; i++) {
			options.push(picker.options[i].value);
		}
		if(options.includes(func))
			picker.value = func;
...
<small>Formatted time input/output is in your timezone unless specified.</small>

<small>The URL de-telemetry-er simply removes all query strings. Additionally,
it converts twitter/x.com URLs to fxtwitter.com, to make embedding easier;
youtu.be is also converted to a regular <code>/watch?v=</code> link.</small>
